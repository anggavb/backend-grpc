...
func (server *Server) createTransfer(c *gin.Context) {
	...
	if !server.validAccount(c, req.FromAccountID, req.Currency) {
		log.Printf("Invalid from account: %d with currency: %s", req.FromAccountID, req.Currency)
		return
	}

	if !server.validAccount(c, req.ToAccountID, req.Currency) {
		log.Printf("Invalid to account: %d with currency: %s", req.ToAccountID, req.Currency)
		return
	}
